...
export class PointTransactionService {
  ...
  async create({ impUid, amount, currentUser }) {
    ...
    // ======================= 3. transaction 시작!! ==================================
    await queryRunner.startTransaction('SERIALIZABLE'); // 이 문법 아래로 try{}catch{}옴. 이걸 기준으로 connection 실행한다고 보면 됨.
    ...
    try {
      ...
      // 2. 유저의 돈 찾아오기
      // 베타락을 걸어서 다른사람이 업데이트 못하게 해야함. 그래야 업데이트 안꼬임. 그리고 필요한 부분에만 써라 안그러면 트래픽 못받을 수 있음ㅠ
      // 글고 락 잘못걸면 큰일남.!!! 자칫하다간 서로 락 걸려서 락이 죽어버림. 데드락이라고 함. 그럼 둘중 하나는 죽여야함ㅠ. 그니까 락거는 순서 일치시켜라. 역행시키지 말고.
